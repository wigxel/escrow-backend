...
export const getUsersEscrowTransactions = (
	...
	return Effect.gen(function* (_) {
		...
		const escrowCount = yield* escrowRepo.escrowCount(filters, currentUser.id);
		const escrowTransactions = yield* escrowRepo.getEscrowTransactions(
			filters,
			currentUser.id,
		);
		...
		return dataResponse({
			...
			meta: {
				...paginate.meta,
				total: escrowCount.count,
				total_pages: Math.ceil(escrowCount.count / paginate.query.pageSize),
			},
		});
	});
};
